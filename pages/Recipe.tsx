...
export const Recipe: React.FC = () => {
  ...
  const handleRefresh = async () => {
     // Recarga simulada de la receta
     await new Promise(resolve => setTimeout(resolve, 800));
     // Aquí se podría volver a fetchear data si fuera API
  };
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: `¡Mira como preparar un ${cocktail.name}!`,
  ...
  const handleBack = () => {
    if (location.state && (location.state as any).fromApp) {
      navigate(-1);
    } else {
      navigate('/');
    }
    ...
    <PullToRefresh onRefresh={handleRefresh}>
      <div className="min-h-screen bg-white pb-24 md:pb-32 animate-fade-in-up">
        <div className="md:flex md:items-start md:min-h-[60vh] max-w-7xl mx-auto">
          ...
          <div className="relative h-[50vh] w-full md:h-[600px] md:w-2/5 md:sticky md:top-0 bg-gray-100 z-0">
            <SmartImage 
              src={cocktail.image} 
              alt={cocktail.name} 
            ...
            {/* Overlay para móviles para asegurar contraste si el botón fuera transparente (aunque ahora es sólido) */}
            <div className="absolute inset-0 bg-gradient-to-b from-black/20 via-transparent to-white md:hidden pointer-events-none"></div>
            
            {/* Navigation - Con z-index alto para asegurar que quede sobre la imagen */}
            <div className="absolute top-6 left-4 right-4 flex justify-between items-start z-30">
              <Button variant="icon" onClick={handleBack} aria-label="Volver">
                <span className="material-icons-round">arrow_back</span>
              </Button>
              <div className="flex gap-3">
                <Button variant="icon" onClick={handleShare} aria-label="Compartir">
                  <span className="material-icons-round">share</span>
                </Button>
                <Button variant="icon" onClick={handleFavClick} className={isFav ? '!bg-toon-primary !border-toon-primary !text-white' : ''} aria-label="Favorito">
          ...
          <div className="mt-20 px-6 max-w-lg mx-auto md:mt-0 md:w-3/5 md:px-12 md:max-w-none md:py-12 z-10 relative">
